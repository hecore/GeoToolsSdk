...
public class QueryLab extends JFrame {
	...
	/**
	 * 在快速启动中，我们使用FileDataStoreFinder连接到特定的文件。这一次，我们将使用更通用的DataStoreFinder，
	 * 它接收连接参数的映射。请注意，相同的代码可用于连接到由DataStoreFactorySpi
	 * （Service Provider Interface）参数指定的完全不同类型的数据存储。
	 * 文件菜单操作使用ShapefileDataStoreFactory或PostgisNGDataStoreFactory的实例来调用此方法。
	 */
	public QueryLab() {  
	       ...
	       pack();  
	       //我们将菜单项和操作添加到文件菜单以连接到shapefile或PostGIS数据库
